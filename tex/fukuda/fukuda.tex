...
\end{numcases}
以上の四つがマクスウェル方程式と呼ばれる、電磁気学全てを支配する方程式達です。尚、文字の意味は$\vb*{E}$：電場、$\vb*{B}$：磁場、$\vb*{j}$：流束、$\vb*{n}$：法線ベクトル、$C$：任意の閉曲線、$S$：任意の閉曲面、$V$：任意の閉曲面Sに囲まれた体積、$\rho$：電荷密度です。\par
...
% \label{eq:potential}
% \int_A^B \vb*{F}\cdot d\vb*{r}=U_B-U_A
% \end{align}
\begin{align}
...
微分と積分の順序を入れ替えて、
\begin{align}
\vb*{B}(\vb*{r})=\frac{1}{4\pi\epsilon_0 c^2}\int_V \nabla\times\frac{\vb*{j}(\vb*{r}')}{|\vb*{r}-\vb*{r}'|}dV'
\end{align}
...
\end{align}
について考える。
$S =S _1 + S_2,\, S_a = S_1 + S_3,\, S_b = S_2 + S_3$として、
\begin{align}
...
よって、
\begin{align}
\int_{S_a} \vb*{h}\cdot\vb*{n}_a dS + \int_{S_b} \vb*{h}\cdot\vb*{n}_b dS = \frac{\partial h_x}{\partial x}dxdydz
\end{align}
$y$軸、$z$軸についても同様に、
\begin{numcases}
  {}
